...
export default function PartieII() {
  ...
  const [stepDone, setStepDone] = useState(() => hasPartieII);

  const autoSendMsg = (() => {
    if (!hasSommaire) return undefined;
    const title    = report.partieIITitle    || "Cadre empirique";
    const chapters = report.partieIIChapters || 2;
    if (hasPartieII) {
      const wc = report.partieII.split(/\s+/).filter(Boolean).length;
      return `Ma Partie II est déjà générée (${wc.toLocaleString("fr-FR")} mots, ${chapters} chapitres, titre : "${title}"). Je suis en mode révision.`;
    }
  ...
  const { messages, send, abort, isThinking, isGenerating, toolCalls, thinkingText } = useStepAgent({
    ...
    onSectionGenerated: (section, content) => {
      if (section === "partie-ii") {
        updateReport({ partieII: content });
  ...
  if (!hasSommaire) {
    return (
      <Layout
        stepName="Partie II"
        stepNumber={8}
        previewPanel={<PreviewPanel activeSection="partie-ii" content="" maxStep={8} isGenerating={false} />}
      >
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200 max-w-sm">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-1">
                La Partie II suit la structure de ton sommaire. Génère d'abord le sommaire à l'étape 5.
              </p>
            </div>
          </div>
          <button
  ...
  return (
    <Layout
      stepName="Partie II — Cadre empirique"
      stepNumber={8}
      previewPanel={
        <PreviewPanel
          activeSection="partie-ii"
    ...
    >
      <div className="flex-1 overflow-y-auto py-4 px-2 md:py-5 md:px-3">
        ...
        {stepDone && !isThinking && !isGenerating && (
          <StepTransitionCard
            title="Partie II générée"
            subtitle="On passe à la conclusion et à la bibliographie."
            onNext={() => { updateReport({ currentStep: 9 }); setLocation("/rapport/step-9"); }}
            nextLabel="Étape 9 : Conclusion"
          />
        )}
        ...
        <ChatInput
          ...
          placeholder={
            isGenerating
              ...
              : stepDone
              ? "Demander une modification de la Partie II…"
