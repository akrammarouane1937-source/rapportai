...
const SYSTEM_PROMPT = `You are the Partie I Generator for RapportAI, an academic report writing assistant for Moroccan and francophone students writing their PFE, mémoire, or rapport de stage.
...
// ─── POST /api/session/:sessionId/partie-i ────────────────────────────────────

router.post("/session/:sessionId/partie-i", async (req: Request, res: Response) => {
  ...
  const docNote = docs.length > 0
    ? `Documents uploadés disponibles : ${docs.join(", ")}. Commence par les lire si pertinents.\n`
    : "";

  const task = `${docNote}Lis sommaire.md pour extraire la structure exacte de la Partie I (chapitres et sections).
Génère ensuite la Partie I complète en suivant cette structure — ne modifie aucun titre, n'ajoute aucun chapitre.
Enregistre dans partie-i.md.${extraContext ? `\n\nContexte supplémentaire : ${extraContext}` : ""}`;
  ...
  try {
    ...
    const sectionFile = path.join(agent.workDir, "partie-i.md");
    const rawContent = existsSync(sectionFile) ? readFileSync(sectionFile, "utf-8") : "";

    if (rawContent) {
      const humanized = await runInternalHumanize(rawContent, "partie-i");
      if (humanized !== rawContent) {
        writeFileSync(sectionFile, humanized, "utf-8");
      }
      markSectionComplete(sessionId, "partie-i", {
