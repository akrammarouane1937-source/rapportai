...
3. Conclusion de la Partie (paragraphe normal)` : "";
  ...
  const schoolProfileBlock = ctx.school ? `\n\n${schoolProfile(ctx.school)}` : "";
  const formattingBlock = buildFormattingPromptBlock(ctx.formatting);

  return `${basePrompt}${memoryContext}${schoolProfileBlock}${formattingBlock}${buildDocumentContext(ctx)}${headingRule}${figuresContext}${skillsContent}`;
}
...
function buildPrompt(ctx: GenerateBody): string {
  ...
  // Student-provided context from the conversation — the single biggest lever against
  // generic output. Must reach EVERY body section, not just the default case.
  const studentCtx = ctx.extraContext?.trim()
    ? `\n\n## CONTEXTE FOURNI PAR L'ÉTUDIANT — À RESPECTER ABSOLUMENT
${ctx.extraContext.trim()}
Ancre tout le contenu sur ces éléments précis (problématique, objectifs, concepts, données). N'utilise PAS de problématique ou de formulation générique.`
    : "";
...
Problématique : ${prob} | Mots-clés : ${kw} | Style de citation : ${style}${studentCtx}
...
Problématique : ${prob} | Style de citation : ${style}${studentCtx}
${buildCitationBlock(ctx.sources, style)}`;
...
Problématique : ${prob} | Mots-clés : ${kw}${studentCtx}
...
Problématique résolue : ${prob}${studentCtx}
