...
### 14. Em Dash — Eliminate Entirely

**Problem:** The em dash (—) is one of the clearest AI writing signals. GPTZero and human readers both flag it. LLMs use it far more than any human writer, and it has become strongly associated with AI-generated text. **Treat every em dash as an error to fix.** Do not merely reduce frequency — remove all of them.

**Replacement rules:**
- Appositive / parenthetical aside → use commas or parentheses
- Dramatic pause before a punchline → split into two sentences
- "That is / namely" construction → use a colon or rewrite the clause
- Explanation tacked on at the end → new sentence or comma
...
**French academic examples:**
> "le cerveau de l'agent — c'est lui qui lit le contexte" → "le cerveau de l'agent : c'est lui qui lit le contexte"
> "les plus capables sont aussi les plus lents — et les plus coûteux" → "les plus capables sont aussi les plus lents et les plus coûteux"
> "Pas par le code — par le langage." → "Pas par le code. Par le langage."
...
**Problem:** GPTZero detects this as the most common French AI sentence structure. Every explanation is delivered with a colon setup followed by a perfectly smooth continuation. Humans break the chain, backtrack, use dashes differently, or just start a new thought.
...
**Problem:** AI inserts parenthetical clauses to add apparent depth. `"Cette tension, ressentie intuitivement par tout investisseur, n'a reçu..."` The parenthetical `"ressentie intuitivement par tout investisseur"` adds nothing factual — it just makes the sentence sound more considered.
...
### 37. Replace, Don't Cut — Preserve Character Count

**Problem:** When removing AI patterns, the natural instinct is to delete the phrase. This produces text that is shorter, thinner, and loses the author's ideas. In academic writing especially, the "fluff" often contains the substance.

**Rule:** When removing an AI-sounding phrase, always replace it with something of similar length:
- A concrete detail or example
- A short punchy sentence that adds a specific observation
- An elaboration of the adjacent idea
- A real-world implication of the concept just stated

**Target:** The humanized output should be within 5% of the original character count.

**Before (cut — wrong approach):**
> "Cette logique d'arbitrage entre capacité et coût se transpose directement à la gestion d'actifs, où certaines tâches sont triviales, comme classer une actualité, tandis que d'autres exigent un raisonnement approfondi."
→ Cut to: "En gestion d'actifs, ce compromis a du sens." *(too short, idea lost)*

**After (replace — correct approach):**
> "En gestion d'actifs, ce compromis prend tout son sens : classer une actualité financière en positif ou négatif ne demande pas le même niveau de raisonnement qu'interpréter un ensemble de signaux de risque contradictoires sur dix positions simultanées." *(same length, idea preserved and sharpened)*

**When elaborating to maintain length, prefer:**
- Making vague examples concrete ("some tasks" → "classifying a news headline")
- Adding a consequence ("which means X fails when Y happens")
- Adding a domain-specific implication ("in portfolio management, this means...")
- A short punchy sentence after a long one ("Pas par le code — par le langage.")
...
7. [ ] Are there any em dashes (—) remaining? → Replace every single one (comma, period, colon, or rewrite).
