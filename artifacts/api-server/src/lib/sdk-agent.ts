...
- ANTI-LISTE (LE PLUS IMPORTANT pour le contenu dense/technique : c'est ce qui fait chuter le score) : transforme TOUTE liste à puces ou liste numérotée noyée dans de la prose en PARAGRAPHES FLUIDES et connectés. Les listes structurées sont le signal d'IA le plus fort dans un mémoire. Intègre les éléments dans des phrases liées par des transitions naturelles et de longueurs variées (ex: au lieu de « 1. Instabilité… 2. Non-normalité… 3. Inefficience… », écris « La première difficulté tient à l'instabilité des paramètres… La distribution des rendements pose un autre problème… Reste l'efficience du marché, … »). Ne garde en liste QUE les vraies énumérations factuelles courtes (ex: une fiche signalétique, une formule).
- ANTI-IA : varie la longueur des PHRASES COMPLÈTES (mélange phrases courtes de 8-12 mots et longues de 25-35), supprime TOUS les tirets cadratins (—), supprime le vocabulaire d'IA, coupe les transitions mécaniques. VARIATION DES DÉBUTS — impérative : JAMAIS deux paragraphes consécutifs qui commencent par le même mot ; au moins la MOITIÉ des paragraphes commencent autrement que par La/Le/Les/L' — par un verbe, un complément, une inversion (ex: « Gérer un portefeuille, c'est… », « Sur la BVC,… », « Quatre difficultés reviennent… », « Harry Markowitz a posé… »). Brise les structures parallèles « X, Y et Z ».
- ANTI-PLAGIAT : change la STRUCTURE SYNTAXIQUE de chaque phrase, alterne voix active/passive, nominalise, remplace les tournures génériques par des synonymes académiques précis, fusionne/scinde les phrases autrement. Aucune phrase ne doit rester reconnaissable telle quelle. Les citations directes entre « » et leurs références (Auteur, année) restent INCHANGÉES.
...
2. Parcours les 37 règles UNE PAR UNE (R1, R3, R4, R7, R8, R9, R10, R12, R13, R14, R23, R26, R27, R28, R30, R31, R32, R33, R35, R36…) + les règles anti-plagiat. Pour CHAQUE règle, scanne tout le texte et repère ce qui la viole ENCORE. EN PRIORITÉ ABSOLUE : (a) toute LISTE à puces/numérotée encore présente dans la prose → convertis-la en paragraphes fluides ; (b) débuts de paragraphes répétitifs → au moins la moitié ne commencent PAS par La/Le/Les/L', jamais deux consécutifs avec le même mot. Puis : phrases de longueur uniforme (4+ longues d'affilée), tirets cadratins (—), vocabulaire d'IA, structures parallèles parfaites, phrases recopiables telles quelles, fragments sans verbe.
...
Conserve 100% du sens, des chiffres, des citations « », des noms propres et acronymes. Ne touche pas à ce qui est déjà conforme. Tout passe par Read/Edit sur "${sectionId}.md".`;
    ...
    for (let pass = 0; pass < MAX_HUMANIZE_PASSES; pass++) {
      if (Date.now() - startedAt > HUMANIZE_DEADLINE_MS) {
        logger.warn({ section: sectionId, pass: pass + 1 }, "humanize: section deadline reached — stopping loop");
        break;
      }
      const isAudit = pass > 0;
      this.abortController = new AbortController();
      const killer = setTimeout(() => {
        try { this.abortController.abort(); } catch { /* already done */ }
      }, PASS_TIMEOUT_MS);
      try {
        for await (const message of query({
        ...
        logger.warn({ err, section: sectionId, pass: pass + 1 }, "humanize: pass failed/timed out — stopping loop, keeping current file");
        clearTimeout(killer);
        break;
      }
      clearTimeout(killer);
...
SOURCES (anti-hallucination) : cite EN PRIORITÉ les documents de la bibliothèque (.txt) et les sources fournies par l'étudiant ; sinon, uniquement des références majeures bien établies que tu connais avec CERTITUDE. N'INVENTE JAMAIS une citation (auteur, année, titre, DOI) — une référence fabriquée fait sanctionner l'étudiant. Si tu n'as pas de source sûre, écris l'affirmation sans citation. Tu n'as PAS accès au web. Style de citation : ${style}.
